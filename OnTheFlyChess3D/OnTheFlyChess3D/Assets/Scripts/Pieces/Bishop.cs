...
{
    private static int[] LEGAL_NOVES_COORDINATE = { -9, -7, 7, 9 };

    public Bishop(int i_piecePosition, PieceColor i_pieceColor, PiecesTpye i_pieceType) : base(i_piecePosition, i_pieceColor, i_pieceType, true) { }

    public Bishop(int i_piecePosition, PieceColor i_pieceColor, PiecesTpye i_pieceType, bool i_isFirstMove) : base(i_piecePosition, i_pieceColor, i_pieceType, i_isFirstMove) { }
    ...
    {
        return new Bishop(i_move.getDestinationCoor(), i_move.getMovedPiece().getPieceColor(), PiecesTpye.Bishop);
    }

    private static bool isFirstColum(int i_currentPosition, int i_dataOffset)
    {
        return BoardUtils.FIRST_COLUM[i_currentPosition] && (i_dataOffset == -9 || i_dataOffset == 7);
    }
    private static bool isEightColum(int i_currentPosition, int i_dataOffset)
    {
        return BoardUtils.EIGHT_COLUM[i_currentPosition] && (i_dataOffset == -7 || i_dataOffset == 9);
    }
}
